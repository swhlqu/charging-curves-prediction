...
class Dataset_generation(Dataset):
    ...
    def load_npy(self):
        self.x_train = np.load("x_train.npy").reshape(-1, 51)
        self.x_valid = np.load("x_valid.npy").reshape(-1, 51)
        self.x_test = np.load("x_test.npy").reshape(-1, 51)
        self.y_train = np.load("y_train.npy").reshape(-1, 51)
        self.y_valid = np.load("y_valid.npy").reshape(-1, 51)
        self.y_test = np.load("y_test.npy").reshape(-1, 51)

    def norm(self):
        self.mean = np.mean(self.x_train, axis=0).reshape(1, 51)
        self.std = np.std(self.x_train, axis=0).reshape(1, 51)
    ...
    def return_dataloader(self):
        self.load_npy()
        self.norm()
        self.x_train = self.x_train.reshape(-1, 51, 1)
        self.x_valid = self.x_valid.reshape(-1, 51, 1)
        self.x_test = self.x_test.reshape(-1, 51, 1)
...
class Exp():
    def __init__(self, enc_in, c_out, d_model, n_heads, e_layers,
        ...
        self.checkpoints_path = './checkpoint/ECMVS'
    ...
    def predict(self, test_loader, name):
        my_file = Path(self.checkpoints_path + '/' + 'checkpoint.pth')
        if my_file.exists():
            best_model_path = self.checkpoints_path + '/' + 'checkpoint.pth'
            self.model.load_state_dict(torch.load(best_model_path))
        ...
        pd.DataFrame(preds).to_csv(name + '_preds.csv', header=False, index=False)
        pd.DataFrame(trues).to_csv(name + '_trues.csv', header=False, index=False)
...
exp = Exp(enc_in=1, c_out=1,
          ...
          distil=True, patience=5000, n=1)
print('>>>>>>>start training>>>>>>>>>>>>>>>>>>>>>>>>>>')
exp.train(train_loader, vali_loader, test_loader, train_epochs=200000)
torch.cuda.empty_cache()
print('>>>>>>>start testing>>>>>>>>>>>>>>>>>>>>>>>>>>')
exp.predict(test_loader, name="test")
exp.predict(vali_loader, name="valid")
exp.predict(train_loader, name="train")
