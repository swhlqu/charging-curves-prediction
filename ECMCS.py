...
class Dataset_generation(Dataset):
    ...
    def load_npy(self):
        self.x_train = np.load("x_train.npy").reshape(-1, 51, 1600)
        self.x_valid = np.load("x_valid.npy").reshape(-1, 51, 1600)
        self.x_test = np.load("x_test.npy").reshape(-1, 51, 1600)
        self.y_train = np.load("y_train.npy").reshape(-1, 51, 1600)
        self.y_valid = np.load("y_valid.npy").reshape(-1, 51, 1600)
        self.y_test = np.load("y_test.npy").reshape(-1, 51, 1600)
    ...
    def return_dataloader(self):
        ...
        for i in range(51):
            x_train = self.x_train[:, i, :].reshape(-1, 1600, 1)
            y_train = self.y_train[:, i, :].reshape(-1, 1600)

            x_valid = self.x_valid[:, i, :].reshape(-1, 1600, 1)
            y_valid = self.y_valid[:, i, :].reshape(-1, 1600)

            x_test = self.x_test[:, i, :].reshape(-1, 1600, 1)
            y_test = self.y_test[:, i, :].reshape(-1, 1600)
...
def adjust_learning_rate(optimizer, epoch, learning_rate, lradj):
    if lradj == 'type1':
        ...
        lr_adjust = {
            1000: 5e-3, 2000: 1e-3, 3000: 5e-4, 4000: 1e-4,
            5000: 5e-5, 6000: 1e-5, 7000: 5e-6, 8000: 1e-6, 9000: 5e-7
        }
...
class Exp():
    def __init__(self, enc_in, c_out, d_model, n_heads, e_layers,
                 d_ff, dropout, activation, distil, patience, n):
        ...
        self.checkpoints_path = './checkpoint/ECMCS-' + str(n)
    ...
    def predict(self, test_loader):
        ...
        preds = preds.reshape(-1, 1600)
        preds = preds
        trues = trues.reshape(-1, 1600)
        trues = trues

        preds_last = np.array(preds_last).reshape(-1, 1600)
        trues_last = np.array(trues_last).reshape(-1, 1600)
        ...
        pd.DataFrame(preds).to_csv(str(self.n) + 'preds.csv', header=False, index=False)
        pd.DataFrame(trues).to_csv(str(self.n) + 'trues.csv', header=False, index=False)
...
data_generation = Dataset_generation(
    path="",
    batch_size=1024)
train, vali, test = data_generation.return_dataloader()

for i in range(51):
    train_loader = train[i]
    vali_loader = vali[i]
    test_loader = test[i]
    exp = Exp(enc_in=1, c_out=1,
              d_model=64, n_heads=4, e_layers=3, d_ff=64 * 4,
              dropout=0.05, activation='gelu',
              distil=True, patience=5000, n=i)
    print('>>>>>>>start training>>>>>>>>>>>>>>>>>>>>>>>>>>')
    exp.train(train_loader, vali_loader, test_loader, train_epochs=20000)
    torch.cuda.empty_cache()
    print('>>>>>>>start testing>>>>>>>>>>>>>>>>>>>>>>>>>>')
    exp.predict(test_loader)
